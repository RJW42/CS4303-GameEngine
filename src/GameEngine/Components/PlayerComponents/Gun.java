...
public class Gun extends Component {
   ...
   public static final int MOUSE_KEY            = 37;
   ...
   public void update() {
      ...
      if(fire_on_key &&
        (sys.input_manager.keys_pressed[FIRE_KEY] || sys.input_manager.mouse_pressed[MOUSE_KEY])){
         fire(new PVector(sys.mouse_x, sys.mouse_y));
      }
   }
   ...
   public static BulletFactory basicBulletFactory(GameEngine sys, GameObject parent){
      return new BulletFactory() {
         @Override
         public Bullet newBullet(PVector spawn_location, PVector spawn_velocity) {
            // Return a basic bullet
            return new Bullet(sys, parent, spawn_location, spawn_velocity, BULLET_RAD, BULLET_MASS);
         }
      };
   }

   public static BulletFactory shotgunBulletFactory(GameEngine sys, GameObject parent){
      return new BulletFactory() {
         @Override
         public Bullet newBullet(PVector spawn_location, PVector spawn_velocity) {
            // Return a basic bullet
            return new Bullet(sys, parent, spawn_location, spawn_velocity, BULLET_RAD, BULLET_MASS * 4);
         }
      };
   }
}
